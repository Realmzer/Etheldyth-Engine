...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var stars:FlxBackdrop = new FlxBackdrop(Paths.image('star'), XY, -0, -0);
		stars.scrollFactor.set();
		stars.screenCenter(X);
		stars.scale.set(0.7,0.7);
		stars.velocity.set(FlxG.random.int(-150, 150),FlxG.random.int(-80, 80));
		stars.antialiasing = ClientPrefs.globalAntialiasing;
        add(stars);

		logoBl = new FlxSprite(100, 1500);
		logoBl.loadGraphic("assets/images/logoeth.png");
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				...
				FlxTween.tween(logoBl,{y: 120}, 3.4, {ease: FlxEase.quintInOut});

				FlxTween.tween(titleText,{y: 30}, 3.4, {ease: FlxEase.expoInOut});
